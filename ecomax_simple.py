...
class Ecomax:
    def __init__(self, rx=21, tx=22, timeout_ms=10000):
        self.uart = UART(1, 115200, timeout=5000, rx=Pin(rx), tx=Pin(tx))
    ...
    def get_data(self):
        ...
        while ticks_diff(deadline, ticks_ms()) > 0:
            ...
            if byte_read == HEADER_START_DELIMITER and not header:
                len_low = ord(self.uart.read(1))
                len_high = ord(self.uart.read(1))
